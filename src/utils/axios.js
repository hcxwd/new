...
// axios.defaults.baseURL = 'http://10.0.1.128:666'; //API 域。默认值：当前域
axios.defaults.baseURL = 'http://testzhongxin.soouu.cn/'; //API 域。默认值：当前域

axios.defaults.withCredentials = true;  // 允许跨域且携带 Cookie（或自定义头）。默认值：false
axios.defaults.timeout = 30000; // 设置请求超时时间（ms）不超过半分钟
axios.defaults.headers.common['token'] = localStorage.getItem('token');  // 携带的自定义头
axios.defaults.headers.post['Content-Type'] = 'application/json';  // 设置请求提内容类型，其他可选值：application/x-www-form-urlencoded, application/json
