...
// 首次执行 npm start 前需要先执行 npm run dll
// 如果 npm run dll 没有执行，就执行它
  // dist/resources/dll/vendor-b7cd9e10c5.js
  // .build/manifest.json
  // .build/webpack-assets.json
const distDir = join(dir, directory.production.envName);
const buildDir = join(dir, directory.build.envName);
const manifest = join(buildDir, 'manifest.json');
const webpackAssets = join(buildDir, 'webpack-assets.json');

if ( !(fs.existsSync(distDir) && fs.existsSync(buildDir) && fs.existsSync(manifest) && fs.existsSync(webpackAssets)) ) {
  execSync('npm run dll');
}

// 读取该文件前，请先确保执行了 dll 命令
